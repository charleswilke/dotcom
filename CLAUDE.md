...
- **How it moves.** The SVG is fetched on hover/focus or on idle, whichever comes first. On click it is placed over the card's screen (layout size times the card's hover `scale`, centred on the image's bounding box, carrying the float's current `rotate`), then a Web Animations run of 48 baked keyframes scales it as 1/(1 - k*t) about the opening's centre while that centre eases to the viewport centre: a camera at constant speed toward a wall. A rAF loop reads the same animation clock to swing the slab (a squeeze along the hinge normal, plus a shade) and push the blocks away and out, and a cream bloom takes the last quarter so a tall or wide viewport never sees past the frame. On finish it writes `sessionStorage['before-times:door-arrival']` and navigates.
