...
- **How it moves.** The SVG is fetched on hover/focus or on idle, whichever comes first. On click it is placed over the card's screen (layout size times the card's hover `scale`, centred on the image's bounding box, carrying the float's current `rotate`), then a Web Animations run of 48 baked keyframes scales it as 1/(1 - k*t) about the opening's centre while that centre eases to the viewport centre: a camera at constant speed toward a wall. A rAF loop reads the same animation clock to swing the slab wider open and to place the blocks by depth: each block carries a `data-depth` (its distance behind the door plane as a fraction of the camera's starting distance, set in the generator) and scales by (1 + d) / (d * s + 1) relative to the poster's scale s, which is exact pinhole perspective, so near blocks keep pace with the frame and far ones lag. No fade; the bloom washes them out. **The hinge is the slab's left edge**, at the opening's right jamb: the door is swung out toward the viewer and to the right, which is why its right edge is the taller one, and the opening stops at the hinge with solid wall behind the slab. Flying through only ever grows the scale across the hinge, sweeping the free edge right and flattening the door against the outer wall. Two wrong versions shipped and were caught on sight: shrinking toward the hinge (reads as the door closing in your face) and flipping across it (a door hinged on the wrong side), and a cream bloom takes the last quarter so a tall or wide viewport never sees past the frame. On finish it writes `sessionStorage['before-times:door-arrival']` and navigates.
