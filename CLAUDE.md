...
- **Two files, one `<picture>`.** Under 768px the streaks were costing the name a third of its size, so `charles-wilke-wordmark-v4-compact.svg` (no lead-in, shorter tail) is served via a `<source media="(max-width: 767px)">`. Change the lettering in both or the phone drifts.
