...
The name in the header is not text. It is `images/charles-wilke-wordmark-v4.svg`, hand-authored vector lettering (one `<g id="glyph-…">` per letter with its own transform, gap masks cutting real transparent joins) rendered as an `<img>` inside the `h1`. v4 (2026-09-12) is the Flash Gordon pass over v3's letterforms: a five-step coral extrusion drawn as stacked `<use>` offsets (it started at nine, and the gutters read as forced until the depth came down with them), a rim-light copy peeking out top-left, lead-in streaks before the C and a fanned speed tail standing 36 units off the last E. The face keeps v3's plain cream gradient: a chrome horizon band was tried across it and rejected on sight as faux chrome, and a hard navy cast shadow under the extrusion went the same way. The face is also stroked 3 units for weight; the gap masks were cut at 10, so the letters don't fuse. v3 is kept in the tree as the flat original.
...
- **Two files, one `<picture>`.** Under 768px the streaks were costing the name a third of its size, so a compact variant is served via a `<source media="(max-width: 767px)">`. The live one is **`charles-wilke-wordmark-v4-compact-notail.svg`** — no lead-in *and* no speed tail, so the lettering fills the full width on a phone. Change the lettering in both it and `charles-wilke-wordmark-v4.svg` or the phone drifts.
  ...
  The tail is self-contained in these files — a `glyph-tail` def, a `pos-tail` placement and one final `<g>` of eight `<use>`s — and no `cut-*` mask references it, which is why it lifts out cleanly.
