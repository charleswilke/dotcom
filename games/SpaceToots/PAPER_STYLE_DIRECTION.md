...
The player ship, the drone, the weaver, Karen, the bomber, and their deaths are restyled behind `PAPER_SAMPLE` in `index.html` (the `PAPER GAMEPLAY SAMPLE` section). Add `?classic` to the URL to get the neon versions back in the same waves, so the two can be compared in play. Nothing in the sample touches a hitbox, a speed, or a spawn.
