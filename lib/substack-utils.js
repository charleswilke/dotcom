...
function summarizeSubstackItem(item) {
    const image = item && item.image ? item.image : null;
    const originalImage = (image && image.url) || (item && item.thumbnail) || '';
    ...
    return {
        ...
        thumbnail: resizeSubstackImage(originalImage, 848),
        image: image ? {
            ...image,
            url: resizeSubstackImage(originalImage, 848),
            srcset: [424, 848, 1272]
                .map(width => `${resizeSubstackImage(originalImage, width)} ${width}w`)
                .join(', ')
        } : null
    };
}
...
async function fetchSubstackItems(limit = 100, options = {}) {
    ...
    return options.enrichVideo === false ? items : enrichVideoEmbeds(items);
}

async function fetchSubstackItemBySlug(slug) {
    const items = await fetchSubstackItems(200, { enrichVideo: false });
    const item = items.find(candidate => slugifyArticle(candidate) === slug);
    if (!item) return null;
    const [enriched] = await enrichVideoEmbeds([item]);
